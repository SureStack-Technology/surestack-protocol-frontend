...
/**
 * @title SureStackToken
 * @dev ERC20Votes token for SureStack Protocol with voting capabilities
 * Used for governance, staking, and as the native token of the protocol
 */
contract SureStackToken is ERC20Votes, Ownable {
    ...
    constructor(address initialOwner) 
        ERC20("SureStack Token", "SST") 
        ERC20Votes()
        EIP712("SureStack Token", "1")
    ...
    function batchTransfer(address[] calldata recipients, uint256[] calldata amounts) 
        ...
        returns (bool) 
    {
        require(recipients.length == amounts.length, "SureStackToken: array length mismatch");
