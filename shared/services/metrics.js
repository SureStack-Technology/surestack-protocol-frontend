...
function getProvider() {
  try {
    ...
    // Fallback to simple provider if resilient setup fails
...
export async function fetchTotalStaked() {
  try {
    ...
    try {
      validatorCount = Number(await contract.validatorCount?.() ?? 0)
    } catch (e) {
      // If validatorCount doesn't exist, we'll need to query events or return 0
      console.warn('[fetchTotalStaked] Could not get validatorCount:', e.message)
      return 0
    }

    if (validatorCount === 0) return 0

    // Fetch all validators and sum staked amounts
    let totalStaked = 0
    const batchSize = 10
    for (let i = 1; i <= validatorCount; i += batchSize) {
      const promises = []
      for (let j = i; j < Math.min(i + batchSize, validatorCount + 1); j++) {
        promises.push(
          contract.validators(j).catch(() => null)
        )
      }
