...
const stakingAbi =
  Array.isArray(ConsensusAndStakingV2ABI?.abi) ? ConsensusAndStakingV2ABI.abi : [];
    ...
    try {
      ...
      await Promise.all(
        addrs.map(async (addr) => {
          try {
            const p = await contract.getValidatorProfile(addr);
      ...
      const finalActiveCount = activeCountFromProfiles > 0 
        ? activeCountFromProfiles 
        : (onChainCount > 0 ? onChainCount : activeCountFromProfiles);
      ...
      setStats(formatted);
      console.log(
        `[useValidatorChainStats] ✅ active=${activeCount} | total=${stakedTotal.toFixed(
          2
