...
import { getHybridWebSocketProvider, getProviderHealth } from '@/shared/rpc/providerManager'
...
function connectWebSocket() {
  const wsMeta = getProviderHealth().ws
  console.info("[EventStream][Diag] WS Provider Info =", wsMeta)
  try {
    const ws = getHybridWebSocketProvider()
    wsProvider = ws

    ws.on?.("open", async () => {
      console.log("[EventStream] Connected to shared WS provider")
      isConnected = true
      reconnectAttempts = 0
    ...
    if (!ws.on) {
      // Provider fallback (likely HTTP). Run listeners immediately with polling semantics.
      setupEventListeners()
      isConnected = true
    }
